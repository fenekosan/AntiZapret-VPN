...
Через AntiZapret VPN (файлы antizapret-\*) работают только (список сайтов автоматически обновляется раз в сутки с 2:00 до 4:00 по времени сервера):
...
**Внимание!** Для правильной работы AntiZapret VPN и полного VPN нужно:
1. Отключить безопасный/частный/персональный DNS в [браузере](https://ya.ru/search/?text=отключить+DNS+в+браузере&promo=force_neuro) и на [устройстве](https://ya.ru/search/?text=отключить+DNS+на+устройстве&promo=force_neuro)
2. В адресной строке браузера открыть и отключить опцию Local Network Access Checks выбрав Disabled:
```
...
```
bash <(wget -qO- --no-hsts --inet4-only https://raw.githubusercontent.com/fenekosan/AntiZapret-VPN/refs/heads/main/setup.sh)
```
...
Внимание! В настоящий момент серверы Аезы могут блокироваться, не рекомендуются к покупке для VPN
...
- Сканирование портов: более 10 попыток подключения с одного IP-адреса по разным портам и протоколам\
- DDoS-атака: более 100 000 новых подключений с одного IP-адреса по разным портам и протоколам\

Если в течение минуты не было попыток подключения - лимиты сбрасываются
При попытке подключения с заблокированного IP-адреса блокировка продлевается до 10 минут\
...
Отключён ответ о неудачной попытке подключения к закрытому порту сервера
