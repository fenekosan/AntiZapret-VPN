...
2. На мобильных устройствах отключить автоматическое/умное переключение между Wi-Fi и мобильной сетью, опция может называться: Wi-Fi+, Intelligent Wi-Fi, Помощь Wi-Fi и тд
...
На сервере в папке /etc/openvpn/server во всех файлах .conf убрать строчки duplicate-cn и setenv duplicate_cn\
...
- Сканирование портов: более 10 новых подключений с одного IP-адреса
...
Подключение по SSH блокируется на минуту с 6-й попытки подключения с одного IP-адреса или суммарно со всех IPv4-адресов подсети /24 или со всех IPv6-адресов подсети /64\
